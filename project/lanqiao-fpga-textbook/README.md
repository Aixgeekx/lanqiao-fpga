...
- 发行版 ZIP：发布仓库根目录 `releases/` 下提供完整包 `lanqiao-fpga-textbook-v0.2.2.zip` 与轻量包 `lanqiao-fpga-textbook-pdf-v0.2.2.zip`
...
- 已补充第16届国赛完整结构化整理、第17届省赛结构化整理、FSM速查表、真题训练索引、真题模拟题训练顺序、训练记录栏、错题复盘模板、`title` 官方资料依据速查表
- 已修复生成脚本中的附录调用、目录页码占位和代码章节排序问题
- 已修复大量扫描 PNG 直接嵌入导致的内存占用问题，题面图会先压缩缓存再进入 PDF 两遍生成流程
- 35个表格全部带"表 N"自动编号，46个题面页全部带"图 N"自动编号
